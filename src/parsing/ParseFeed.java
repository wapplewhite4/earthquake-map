...
public class ParseFeed {
	...
	//loads top 20 earthquakes by magnitude from CSV
	public static List<PointFeature> loadTop20QuakesFromCSV(PApplet p, String fileName) {
		
		List<PointFeature> top20Quakes = new ArrayList<PointFeature>();
		...
		for (String row : rows) {
			...
			if (!columns[0].equals("time")) {
				PointFeature point;
				float lat = Float.valueOf(columns[1]);
				float lon = Float.valueOf(columns[2]);
			...
			for (int i = 0; i < columns.length - 1; i++) {
				...
				if (i == 13) {
					point.putProperty("title", columns[i]);
					System.out.println(columns[i]);
				}
				if (i == 4) {
					point.putProperty("magnitude", columns[i]);
					//System.out.println(columns[i]);
				}
				if (i == 3) {
					float depth = Float.parseFloat(columns[i]);
