...
public class EarthquakeCityMap extends PApplet {
	...
	private void selectMarkerIfHover(List<Marker> markers)
	{
		...
		for (Marker m : markers) 
		{
			...
			if (m.isInside(map,  mouseX, mouseY)) {
				lastSelected = (module5.CommonMarker)m;
				m.setSelected(true);
				return;
			}
		}
	...
	//helper method to check if Earthquake has been clicked
		...
		for (Marker m : quakeMarkers) {
			...
			if (!marker.isHidden() && marker.isInside(map, mouseX, mouseY)) {
				...
				for (Marker hide : cityMarkers) {
					if (hide.getDistanceTo(marker.getLocation()) 
							> marker.threatCircle()) {
						hide.setHidden(true);
					}
				}
	...
	//helper method to check if City has been clicked
